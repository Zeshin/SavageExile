	...
	if ((_fSize select 0) > 0) then {
		_cargoGrp = createGroup _side;
			...
			0=[_cargoGrp,"INFskill"] call eos_fnc_grouphandlers;
				...
				null = [_mkr,_fGroup,_counter] execvm "addons\eos\functions\TransportUnload_fnc.sqf";
	...
	sleep 1;
			...
			{			
					...
					_mkr setmarkeralpha _mAN;
						...
						if (_eosZone) then {
null = [_mkr,[_PApatrols,_PAgroupSize],[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,0,0,0],[_faction,_mA,350,_CHside]] execVM "addons\eos\core\EOS_Core.sqf";
...
null = [_mkr,[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups],[_CHGroups,_fSize],_settings,[_pause,_waves,_timeout,_eosZone,_hints],true] execVM "addons\eos\core\b_core.sqf";
						};};
